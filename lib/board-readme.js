...
Assignment is one \`agent:\` value — frontmatter \`agent: codex\` here, an
\`agent:codex\` label on GitHub. It names who acts NEXT, not who did the work;
exactly one holder at a time. Set it when you hand off (implementer →
reviewer → owner) and log the handoff (\`agent: codex → claude — PR #53 ready
for review\`). Schedulers pick up work by filtering on their own name.

Keep bodies lean. No priorities or milestones — if ordering matters, say so
in prose. Never edit \`created:\`. New task = new file: \`slug-of-title.md\`
with status/created frontmatter and a \`# Title\` heading.
`;
