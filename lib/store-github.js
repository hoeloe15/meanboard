...
export async function createGithubStore({ repo, token, fetchImpl = fetch, pollMs = 6000 }) {
  ...
  const toTask = async (issue, extraEntries = [], prs) => {
    ...
    const body = entries.length
      ...
      : spec;
    const task = { id: String(issue.number), ref: `#${issue.number}`, title: issue.title, status: statusOf(issue), created: stamp(issue.created_at), body };
    if (prs) task.prs = prs;
    return task;
  };

  // Linked-PR context: PRs that cross-reference the issue or are mentioned by
  // URL in its body/comments. Their conversation and review verdicts merge
  // into the task's log so one view carries the whole story.
  async function prContext(issue) {
    ...
    const entries = [], prs = [];
    for (const number of [...numbers].slice(-4)) {
