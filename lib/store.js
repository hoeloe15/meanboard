...
export const validAgent = agent => typeof agent === 'string' && /^[\w.-]{0,40}$/.test(agent);
...
export function parseTask(text, filename = 'task.md') {
  ...
  return { id, title: heading?.[1].trim() || id, status: meta.status || 'draft', agent: meta.agent || '',
    created: meta.created || '', body: heading ? content.slice(heading.index + heading[0].length) : content };
}

export function serializeTask({ title, status = 'draft', agent = '', created, body = '' }) {
...
// Set (or, with an empty value, remove) one frontmatter field in place.
function setField(text, key, value) {
  const fm = frontmatter(text);
  const eol = eolOf(text);
  if (!fm) return value === '' ? text : `---${eol}${key}: ${value}${eol}---${eol}${text}`;
  const pattern = new RegExp(`^([ \\t]*${key}[ \\t]*:[ \\t]*)[^\\r\\n]*(\\r?\\n|$)`, 'm');
  const match = pattern.exec(fm.raw);
  if (match) {
    const at = fm.start + match.index;
    if (value === '') return text.slice(0, at) + text.slice(at + match[0].length);
    return text.slice(0, at) + match[1] + value + match[2] + text.slice(at + match[0].length);
  }
  if (value === '') return text;
  return text.slice(0, fm.start) + `${key}: ${value}${eol}` + text.slice(fm.start);
}
...
export async function updateTask(dir, id, patch) {
  ...
  return enqueue(id, async () => {
    ...
    if (patch.status !== undefined) text = setField(text, 'status', patch.status);
    if (patch.agent !== undefined) text = setField(text, 'agent', patch.agent);
